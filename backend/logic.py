...
def process_upload(file_content: bytes) -> pd.DataFrame:
    ...
    for encoding in encodings:
        ...
        for sep in separators:
            try:
                logger.warning(f"⚠️ Strict parsing failed. Retrying with on_bad_lines='skip', encoding={encoding}, sep='{sep}'")
        ...
        if target_col not in df.columns:
            for alias in aliases:
                if alias in df.columns:
                    ...
                    logger.info(f"✅ Mapped column '{alias}' -> '{target_col}'")
                    break
    
    # Print available columns for debugging
    logger.info(f"📋 Available columns after normalization: {list(df.columns)}")
